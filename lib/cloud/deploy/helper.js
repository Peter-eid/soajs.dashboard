		...
		function getClusterResource(cb) {
			...
			model.findEntry(soajs, opts, (error, resource) => {
				utils.checkErrorReturn(soajs, cb, { config: config, error: error, code: 600 }, () => {
					...
					let protocol =  resource.config.URLParam && resource.config.URLParam.protocol ? resource.config.URLParam.protocol : 'http';
					if (resource.config.credentials && resource.config.credentials.username && resource.config.credentials.password) {
					...
					context.variables['$SOAJS_ANALYTICS_ES_PROTOCOL'] = protocol;
					...
					async.eachOf(resource.config.servers, (oneServer, index, callback) => {
						if (context.catalog && context.catalog.subtype && context.catalog.subtype === 'logstash'){
							oneServer.host = protocol + "://" + oneServer.host;
						}
