...
var helpers = {
	...
	getAnalyticsEsInfo: function (soajs, config, context, model, cb) {
		...
		let analyticsClusterName;
		model.findEntry(soajs, opts, (error, settings) => {
			utils.checkErrorReturn(soajs, cb, {config: config, error: error, code: 600}, () =>{
				let es_analytics_db;

				if (settings && settings.elasticsearch) {
					// adding computed environment variables for ES security
					if(settings.elasticsearch.security && Object.keys(settings.elasticsearch.security).length > 0) {
						if(settings.elasticsearch.security.owner) {
							context.variables["$SOAJS_ANALYTICS_ES_USER_OWNER"] = settings.elasticsearch.security.owner;
						}
						if(settings.elasticsearch.security.rw) {
							context.variables["$SOAJS_ANALYTICS_ES_USER_RW"] = settings.elasticsearch.security.rw;
						}
						if(settings.elasticsearch.security.ro) {
							context.variables["$SOAJS_ANALYTICS_ES_USER_RO"] = settings.elasticsearch.security.ro;
						}
					}

					if(settings.elasticsearch.db_name && settings.elasticsearch.db_name !== '') {
						es_analytics_db = settings.elasticsearch.db_name;
					}
				}

				if(!es_analytics_db) return cb();

				let envRecord = context.envRecord;
				...
				if(envRecord.dbs && envRecord.dbs.databases && envRecord.dbs.databases[es_analytics_db] && envRecord.dbs.databases[es_analytics_db].cluster){
					analyticsClusterName = envRecord.dbs.databases[es_analytics_db].cluster;
				}

				if(analyticsClusterName) {
					return getClusterResource(cb);
				}
				else {
					opts.collection = colls.environment;
					opts.conditions = {};
					model.findEntries(soajs, opts, function (error, envRecords) {
						utils.checkErrorReturn(soajs, cb, { config: config, error: error, code: 600 }, function() {
							async.each(envRecords, function (oneEnvRecord, callback) {
								if(oneEnvRecord.code === envRecord.code) {
									return callback();
								}

								if(oneEnvRecord.dbs && oneEnvRecord.dbs.databases && oneEnvRecord.dbs.databases[es_analytics_db] && oneEnvRecord.dbs.databases[es_analytics_db].cluster){
									analyticsClusterName = oneEnvRecord.dbs.databases[es_analytics_db].cluster;
								}

								return callback();
							}, function() {
								if(!analyticsClusterName) return cb();
		...
		function getClusterResource(cb) {
			//get analytics cluster resource
			opts.collection = colls.resources;
			opts.conditions = {
				name: analyticsClusterName,
				type: 'cluster',
				category: 'elasticsearch'
			};
			model.findEntry(soajs, opts, (error, resource) => {
				utils.checkErrorReturn(soajs, cb, { config: config, error: error, code: 600 }, () => {
					if(!resource || !resource.config || Object.keys(resource.config).length === 0) {
						return cb();
					}

					if (resource.config.credentials) {
						if (resource.config.credentials.username) {
							context.variables['$SOAJS_ANALYTICS_ES_USERNAME'] = resource.config.credentials.username;
						}
						if (resource.config.credentials.password) {
							context.variables['$SOAJS_ANALYTICS_ES_PASSWORD'] = resource.config.credentials.password;
						}
					}
					if (resource.config.URLParam) {
						if (resource.config.URLParam.protocol) {
							context.variables['$SOAJS_ANALYTICS_ES_PROTOCOL'] = resource.config.URLParam.protocol;
						}
					}

					if(resource.config.servers && Array.isArray(resource.config.servers)) {
						context.variables["$SOAJS_ANALYTICS_ES_NB"] = resource.config.servers.length;
					}

					async.eachOf(resource.config.servers, (oneServer, index, callback) => {
						if (context.catalog && context.catalog.subtype && context.catalog.subtype === 'logstash'
						&& context.variables['$SOAJS_ANALYTICS_ES_PROTOCOL']){
							oneServer.host = context.variables['$SOAJS_ANALYTICS_ES_PROTOCOL'] + "://" + oneServer.host;
						}
