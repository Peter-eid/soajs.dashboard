...
var BL = {
	 ...
	 *
	 */
	listResourcesDatabases: function (config, req, res, cb) {
		var opts = {
			collection: resourcesColName,
			conditions: {
				type: "cluster",
		...
		if (req.soajs.inputmaskData.category) {
			opts.conditions.category = req.soajs.inputmaskData.category;
		}
		BL.model.findEntries(req.soajs, opts, function (error, resources) {
			utils.checkErrorReturn(req.soajs, cb, {config: config, error: error, code: 600}, function () {
				var opts = {};
				...
				async.map(resources, function (oneResource, callback) {
					return callback(null, oneResource.name);
				}, function (error, clusters) {
					BL.model.findEntry(req.soajs, {
						"collection": envColName,
						"conditions": {'code': req.soajs.inputmaskData.env.toUpperCase()},
						"fields": {"dbs.databases": 1}
					}, function (err, envRecord) {
						utils.checkErrorReturn(req.soajs, cb, {
							config: config,
							error: err || !envRecord,
							code: 402
						}, function () {
							var dbs = Object.keys(envRecord.dbs.databases);
							async.filter(dbs, function (oneDb, callback) {
								callback(null, clusters.indexOf(envRecord.dbs.databases[oneDb].cluster) !== -1);
							}, cb);
						});
					});
				});
